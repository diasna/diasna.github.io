...
class ChatLogic {
    ...
    async detectIntent(message) {
        try {
            ...
            const intentPrompt = `You are an intent router. Decide if a user message requires one tool call.

    TOOLS (use only when explicitly appropriate):
    - get_weather(city: string)
    Use only for current/near-term weather in a specific place (e.g., "now", "today", "forecast", "temperature in <city>").
    Do NOT use for climate, historical, generic descriptions, or explanations.
    If city not clearly provided, output NONE.

    - get_stock_price(symbol: string)
    Use only for current/near-term equity price when the user explicitly mentions a ticker or "stock/share/quote/price" context.
    If no clear ticker/symbol or it’s not about current price, output NONE.
    Normalize symbol to uppercase.

    - search_web(query: string)
    Use only when the user clearly asks to "search", "look up", "find", or requests "latest/current/recent/today/this week" news/info about a topic/person/company.
    Do NOT use for definitions, explanations, summaries, jokes, opinions, or general knowledge that doesn't need freshness.

    - get_current_time(timezone?: string)
    Use when the user asks for the current time/date ("what time is it", "time in …", "current date").
    Only set timezone if the user explicitly provides a valid IANA zone (e.g., "Asia/Jakarta", "America/New_York", "UTC").
    If no valid timezone provided, use {}.

    OUTPUT FORMAT (strict):
    - Return EXACTLY one of:
    1) A minified JSON object: {"action":"<tool_name>","parameters":{...}}
    2) The word: NONE
    - No markdown/code fences, no prose, no extra text.

    CONSERVATIVE RULES:
    - When in doubt → NONE.
    - Never invent missing parameters.
    - Never explain tools or how to call them.
    - Prefer NONE for jokes, stories, chit-chat, explanations, generic “what is/who is”, tutorials, and brainstorming.

    User message: "${message}"

    Examples:
    - "weather in Tokyo now" → {"action":"get_weather","parameters":{"city":"Tokyo"}}
    - "temperature in Tangerang today" → {"action":"get_weather","parameters":{"city":"Tangerang"}}
    - "AAPL stock price" → {"action":"get_stock_price","parameters":{"symbol":"AAPL"}}
    - "tsla quote today" → {"action":"get_stock_price","parameters":{"symbol":"TSLA"}}
    - "search latest news about AI" → {"action":"search_web","parameters":{"query":"latest news about AI"}}
    - "look up Dias Nurul Arifin" → {"action":"search_web","parameters":{"query":"Dias Nurul Arifin"}}
    - "what time is it" → {"action":"get_current_time","parameters":{}}
    - "time in Asia/Jakarta" → {"action":"get_current_time","parameters":{"timezone":"Asia/Jakarta"}}
    - "tell me a joke" → NONE
    - "who is Albert Einstein" → NONE
    - "explain quantum computing" → NONE
    - "write a story about a cat" → NONE

    Respond with ONLY the JSON object or NONE.
    ...
    Response:`;
            ...
            const completion = await chatState.webllmEngine.chat.completions.create({
                messages: [
                    { role: "system", content: "You strictly output either a single minified JSON object or the single word NONE. No code fences. No explanations. No extra text." },
